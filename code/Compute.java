...
public class Compute {
	...
	//constructor
	public Compute (){
		this.computePi();this.computeLn2();
	}
	...
	private int [] getFraction (double x){
		...
		for (int i=0; i <temp.length(); i++){
			if (temp.charAt(i) == '-')sign="-";//reassign sign
			if (temp.charAt(i) == '.'){flag=true;continue;}
			if (flag){
		...
		if (Integer.parseInt(fractional) == 0){//no fraction
			return new int []{decimal, 1};
		}
		//System.out.println (fractional);
		String tens="1";
		for (int i=0; i < fractional.length(); i++){tens+= "0";}
	...
	private  double nthroot(int n, double x) {
		if(n % 2 ==0 && x< 0){System.out.println ("Error! can not compute root...");System.exit(0);};
		int np = n - 1;
		double g1 = x;
